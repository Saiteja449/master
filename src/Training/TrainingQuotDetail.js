...
import { Text, View, Image, ScrollView, TouchableWithoutFeedback, Modal, TextInput, TouchableOpacity, FlatList, Alert, Dimensions } from 'react-native';
...
const TrainingQuoteDetails = ({ route }) => {
  ...
  useEffect(() => {
    ...
    setExtraDays(item.extra_sessions)
  ...
  const createQuotationApi = async () => {
    ...
    console.warn("ADDONSSSSSS ::: ", missingSkills)
    // setSkillpopup(true)
    // return
    //M22IQ6Z3VQQHT
    // return
  ...
  const addExtraSkills = async () => {
    ...
    console.warn("PPPPPPPPPPPPP DATAAAA", data)
    try {
      const url = `${API_BASE_URL}provider/addExtraSkills`;
      const response = await fetch(url, {
        method: "POST",
        headers: {
          'Authorization': `Bearer ${userData.token}`,
            ...
            {packageDetail && (
              <View>
                ...
                <View style={[globle_Style.pakg_time_con, { borderTopWidth: 0, borderBottomWidth: 1 }]}>
                  <View style={[globle_Style.pakg_time_itm, globle_Style.edit_optn, { justifyContent: 'flex-start' }]}>
                    <View style={[globle_Style.pakg_time_lst, { flexDirection: 'row', justifyContent: 'space-between', }]}>
                      ...
                      <Text style={globle_Style.quot_dtls_hd}>{item.package_price}</Text>
                    </View>
                  </View>
                ...
                <View style={globle_Style.pakg_incld}>
                  ...
                  <View style={[globle_Style.radio_con, { marginVertical: 20 }]}>
                    <TouchableWithoutFeedback onPress={() => handleRadio(1, 'every_day', setTrainingMethodRadio, setTrainingMethod)}>
                      <View style={[globle_Style.radioWapper, { borderRadius: 40 }, trainingmethodRadio === 1 ? globle_Style.active_wrapper : globle_Style.radioWapper, { borderRadius: 40, flexDirection: 'column', justifyContent: 'center', paddingHorizontal: 0, flexShrink: 1, flexGrow: 0 }]}>
                        <View style={globle_Style.stardp_con}>
                          <Text style={globle_Style.rdo_txt}>Every day</Text>
                        </View>
          ...
          <View style={[globle_Style.skilltrain]}>
            <View style={globle_Style.skillpopup}>
              <View style={globle_Style.skillpopup_con}>
                      ...
                      return (
                        ...
                        >
                          <Text style={[
                            ...
                            isActive && { color: 'white' }, item.length > 12 ? { maxWidth: screenWidth * 0.2, flexShrink: 1 } : null
                          ]} numberOfLines={0} ellipsizeMode='tail'>
                            Over Excitement
                          </Text>
                          <Image
                            source={
                ...
                {skillError && <Text style={[globle_Style.errorText, { paddingHorizontal: 5, }]}>Please select skills</Text>}
                <View style={[]}>
                  ...
                  <View style={globle_Style.skill_update}>
                    ...
                    <View style={globle_Style.popbtn}>
                      <TouchableOpacity onPress={() => {
                        addExtraSkills()
                        console.warn("SELECTEDD SKILLSSSS :: ", selectedSkills)
                      }}>
...
export default TrainingQuoteDetails;
