...
const QuoteDetails = ({ route }) => {
  ...
  const isDistanceMoreThanFiveKm = (distanceStr) => {
    const distance = parseFloat(distanceStr);
    console.warn("Distance in KM :: ", distance)
    return distance > 5;
  };
  ...
  return (
    <ScrollView style={globle_Style.container}>
      <View style={globle_Style.quot_dtls_sec}>
        ...
        <TouchableWithoutFeedback onPress={() => {
          if (isDistanceMoreThanFiveKm(item.distance)) {
            Alert.alert(
              "Alert!!",
              "This booking is more than 5 km away from your location. Are you sure you want to send a bid?",
              [
                {
                  text: "Cancel",
                  style: "cancel"
