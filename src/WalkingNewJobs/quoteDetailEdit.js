...
const QuoteDetailEdit = ({ route }) => {
  ...
  const [bookingAddOns, setBookingAddOns] = useState(item.bookingAddOns || []);
  ...
  const handleSelectItem = (date) => {
    setPreferableTime((prevSelectedDates) => {
      ...
      // ✅ If only one selection allowed, always replace with new date
      if (maxSelections === 1) {
        return [date];
      }
      ...
      if (prevSelectedDates.length >= maxSelections) {
        return prevSelectedDates;
      }

      // Enforce 60-min gap if walk_duration is "60 min walk"
      if (item.walk_duration === '60 min walk') {
        const selectedMinutes = convertToMinutes(date);

        const hasConflict = prevSelectedDates.some((selectedDate) => {
          const diff = Math.abs(convertToMinutes(selectedDate) - selectedMinutes);
          return diff < 60;
        });

        if (hasConflict) {
          Alert.alert("OOPS!!", "Please select times at least 60 minutes apart.");
          return prevSelectedDates;
        }
      }
  ...
  //     // ✅ Check if max selections reached first
  //     if (prevSelectedDates.length >= maxSelections) {
  //       return prevSelectedDates;
  //     }

  //     // ✅ Enforce 60-min gap only if walk_duration is "60 min walk"
  //     if (item.walk_duration === '60 min walk') {
  //       const selectedMinutes = convertToMinutes(date);
  ...
  useEffect(() => {
    ...
    const initialPrices = bookingAddOns.map(bookingAddOn => {
  ...
  //     } else {
  //       // Find the price for the addon by looking it up in addons
  ...
  //       return [...prevSelected, { name, price }];
  //     }
  //   });
  // };
  ...
  useEffect(() => {
    ...
    const updatedBidingAmount = (updatedTotalAmount - (updatedTotalAmount * (parseInt(discountPercentage || 0, 10) / 100))).toFixed(2);
  ...
  //   const updatedTotalAmount = (parseInt(item.total_price, 10) + parseInt(addOnsTotal, 10)).toFixed(2);
  //   setTotalAmount(updatedTotalAmount);

  //   const updatedBidingAmount = (updatedTotalAmount - (updatedTotalAmount * (parseInt(discountPercentage) / 100))).toFixed(2);
  //   setBidingAmount(updatedBidingAmount);
    ...
    if (isEdit) {
      ...
      try {
        ...
        if (result.status == true) {
          Alert.alert("Success!!", "Quotation has been edited successfullyy.", [
      ...
      try {
        ...
        if (result.status == true) {
          Alert.alert("Success!!", "Quotation has been sent successfullyy.", [
            { text: "OK", onPress: () => navigation.goBack() }
          ]);
          setIsDisabled(false);
          console.warn("SAHILLL222", result)
  ...
  const petRenderItem = ({ item }) => {
    return (
      <View style={[globle_Style.acnt_con, globle_Style.walk_info_con2, { borderTopWidth: 1, }]}>
        ...
        <View style={[globle_Style.acnt_lst, globle_Style.walk_con_lst]}>
          <View style={[globle_Style.acnt_itm, globle_Style.ser_dtl_lst]}>
            <Image source={require('../../assets/images/breed.png')} />
            <Text style={globle_Style.servc_pac_itm_txt}>Breed : {item.breed}</Text>
          </View>
        </View>
        <View style={[globle_Style.acnt_lst, globle_Style.walk_con_lst]}>
          <View style={[globle_Style.acnt_itm, globle_Style.ser_dtl_lst]}>
      ...
      <View style={globle_Style.quot_dtls_sec}>
        <View style={globle_Style.walk_info_sec}>
          <View style={[globle_Style.receive_info_sec, globle_Style.walk_con_top]}>
            <TouchableOpacity onPress={showPetDetails}>
                  ...
                  <View style={globle_Style.pakg_totl_con}>
                    ...
                    <View style={globle_Style.discont_opt}>
                      ...
                      <TextInput
                        ...
                        editable={true}
                        keyboardType="numeric"
                        value={discountPercentage} onChangeText={(text) => {
