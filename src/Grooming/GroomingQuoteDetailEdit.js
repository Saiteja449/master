...
const GroomingQuoteDetailEdit = ({ route }) => {
  ...
  const toggleCheckbox = (name) => {
    setAddOnPrices(prevSelected => {
      ...
      if (isSelected) {
        ...
        // Find the addon in previously selected list
        const existingAddon = prevSelected.find(addon => addon.name === name);

        // If it was previously selected, retain its last known price
        if (existingAddon) {
          return [...prevSelected, { name, price: existingAddon.price }];
        }

        // Otherwise, find the price from selectedAddOns
        const addon = bookingAddOns.find(addon => addon.name === name);
        const price = addon ? addon.price : '00';
...
export default GroomingQuoteDetailEdit;
