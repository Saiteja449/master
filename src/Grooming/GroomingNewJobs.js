...
const GroomingNewJobs = () => {
  ...
  const fetchNewJobsApiData = async () => {
    ...
    try {
      ...
      if (result.status == true) {
        ...
        console.warn("RESULTTTTTTT :: ", result)
  ...
  const fetchMyQuoteApiData = async () => {
    const url = `${API_BASE_URL}provider/${userData.id}/myQuotes/2`;
  ...
  const fetchActiveJobsApiData = async () => {
    const url = `${API_BASE_URL}provider/${userData.id}/activeJobs/2`;
      ...
      <View style={globle_Style.newjob_sec}>
        <View style={globle_Style.newjob_con}>
          ...
          <View style={globle_Style.newjob_btn}>
            ...
            <TouchableWithoutFeedback onPress={() => navination.navigate('GroomingAllQuotes', { item: item, showmyquote: false })}>
              <Text style={[globle_Style.white_btn]}>Total Quotes {item.totalQuotes}</Text>
            </TouchableWithoutFeedback>
            <TouchableOpacity onPress={() => navination.navigate('GroomingQuoteDetails', {
              item: item,
              isEdit: false
            })}>
  ...
  //           <View style={globle_Style.newjob_txt_con}>
  //             <View style={globle_Style.newjob_txt_lft}>
  //               <Text style={globle_Style.job_exp}>{item.package_name},<Text style={[globle_Style.job_exp, globle_Style.job_day]}>{item.service_frequency}</Text></Text>
  //               <View style={globle_Style.job_distc}>
  //                 <View style={globle_Style.newjob_txt_rgt}>
  //                   <DistaneMap />
  //                   <Text style={globle_Style.distc_txt}>{item.distance} away</Text>
      ...
      <View style={[globle_Style.newjob_con, { marginHorizontal: 16, marginTop: 5 }]}>
        <View style={globle_Style.newjob_lst}>
          ...
          <View style={globle_Style.newjob_itm_rgt}>
            <View style={globle_Style.newjob_txt_con}>
              <View style={globle_Style.newjob_txt_lft}>
                <Text style={globle_Style.job_exp}>{packageNames}</Text>
                <View style={globle_Style.job_distc}>
                  <View style={globle_Style.newjob_txt_rgt}>
                    <DistaneMap />
        ...
        <View style={[globle_Style.newjob_btn, { paddingRight: 0, alignItems: 'center', justifyContent: 'flex-start' },]}>
          ...
          <TouchableWithoutFeedback onPress={() => navination.navigate('GroomingAllQuotes', { item: item, showmyquote: true })}>
            <Text style={[globle_Style.white_btn, { paddingHorizontal: 12, }]}>Total Quotes {item.totalQuotes}</Text>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback>
            <TouchableOpacity onPress={() => navination.navigate('GroomingQuoteDetailEdit', {
              item: item,
              isEdit: true
            })}>
      ...
      <View style={[globle_Style.newjob_btn, globle_Style.actjob_btn]}>
        <TouchableWithoutFeedback>
          <View style={globle_Style.walk_time}>
            <Text style={globle_Style.walk_time_txt_strt}>Grooming {item.track_status}</Text>
            {/* <Text style={[globle_Style.walk_time_txt_strt, globle_Style.walk_time_txt]}>{item.service_time}</Text> */}
          </View>
        </TouchableWithoutFeedback>

        <TouchableWithoutFeedback onPress={() => { if (item.track_status == 'terminate') return; handleGrooming(item.booking_id, item.pet_id, item.track_status, item) }} >

          <LinearGradient colors={['#FBAB51', '#FE8705']} start={{ x: 0, y: 1 }} style={[globle_Style.actjob_btn_lin, item.track_status != null && (item.track_status == 'terminate') ? { opacity: 0.5 } : { opacity: 1 }]}>
            <Text style={[globle_Style.gbl_btn_two, globle_Style.newactjob_btn]}>{item.track_status == 'in_progress' || item.track_status == 'completed' || item.track_status == 'rejected' ? 'Update Grooming' : item.track_status == 'terminate' ? 'Terminated' : 'Start Grooming'}</Text>
          </LinearGradient>
    ...
    <View style={[globle_Style.active_servc_sec,]}>
      ...
      <View style={globle_Style.act_serv_exted}>
        <View style={globle_Style.actv_serv_rgt}>
          <TouchableWithoutFeedback onPress={() => navination.navigate('GroomingActiveJobDetail', { item: item })}>
  ...
  const renderItemActiveUpcoming = ({ item }) => (
    ...
    <View style={[globle_Style.active_servc_sec,]}>
      <View style={globle_Style.serv_walk_act}>
        <FlatList
          data={item.pets}
          renderItem={renderItemmmUpcoming}
        />
      </View>
      <View style={globle_Style.act_serv_exted}>
        <View style={globle_Style.actv_serv_rgt}>
          <TouchableWithoutFeedback onPress={() => navination.navigate('ActiveJobDetail', { item: item })}>
            <View style={globle_Style.call_menu}>
              <Text style={[globle_Style.view_all]}>View Details</Text>
            </View>
          </TouchableWithoutFeedback>
...
export default GroomingNewJobs;
