...
import { View, StyleSheet, PermissionsAndroid, Platform, TouchableOpacity, Text, ActivityIndicator, BackHandler, Button, Alert, Modal, TouchableWithoutFeedback, Image } from 'react-native';
...
const WalkTracking = ({ route }) => {
            ...
            const watchId = Geolocation.watchPosition(
                (position) => {
                    ...
                    if (uniqueKey) {
                        ...
                        console.warn("Unique key is not yet available, skipping waypoint update.");
                    }
                },
                (error) => {
                    console.error('Error fetching location: ', error);
    ...
    const saveWaypointsToFirebase = (updatedWaypoints) => {
        ...
        ref.update({ waypoints: updatedWaypoints })
            .then(() => console.warn('Waypoints updated in Firebase.', uniqueKey))
            .catch((error) => console.error('Error saving waypoints to Firebase: ', error));
    };
    ...
    const saveDataToFirebase = (data, key) => {
        ...
        ref.set(data)
            ...
            .catch((error) => console.error(`Error saving ${key} to Firebase: `, error));
    };

    const getRouteDistance = async (origin, waypoints, destination) => {
        try {
            const waypointsParam = waypoints.map(point => `${point.latitude},${point.longitude}`).join('|');
    ...
    // Stop the stopwatch
    const stopStopwatch = () => {
        console.warn('END WALK CLICKKKKKKKK :: ', totalDistance, formatTime(elapsedTime))

        Alert.alert(
            "End Walk!!!",
            "Are you sure you want to complete the walk?",
            [
                { text: "Yes", onPress: () => handleManageEndWalk(data.service_time, data.booking_id, data.pet_id) },
    ...
    const handleEndWalkAPi = async (selectedTime, selectedBookingId, selectedPetId) => {
        try {
    ...
    return (
        ...
        <>
            ...
            {loader ? (<View style={globle_style.loader}><ActivityIndicator size="large" color="#0000ff" /></View>)
                :
                        ...
                        <View style={[globle_style.overlay, { paddingHorizontal: 0, }]}>
                            ...
                            <View style={[globle_style.walk_type_popup]}>
                                ...
                                <View style={globle_style.walk_typ_pop_hd}>
                                    <Text style={[globle_style.my_prof_txt, globle_style.popup_txt_hd]}>Tracking Details</Text>
                                </View>

                                <View style={[globle_style.walk_typ_pop_hd, { flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }]}>
                                    <Image source={require('../../assets/images/lightbulb.png')} style={{ width: 20, height: 20 }} />
                                    <Text style={[styles.statLabel, { fontSize: 12 }]}>End the walk or get client approval to ensure payment</Text>
                                </View>

                                <View style={[globle_style.walk_typ_pop_hd, { marginBottom: 16, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }]}>
                                    <Image source={require('../../assets/images/lightbulb.png')} style={{ width: 20, height: 20 }} />
                                    <Text style={[styles.statLabel, { fontSize: 12 }]}>Keep the app open during tracking</Text>
                                </View>



                                <View style={[globle_style.pet_add_forms, globle_style.walk_typ_frm]}>
                                    <View style={globle_style.details}>
                                        <View style={globle_style.rdo_btn}>
                                            <View style={[globle_style.form_group, { flexDirection: 'column' }]}>
                    ...
                    <Modal
                        ...
                        <View style={globle_style.popup}>
                            <View style={[globle_style.overlay]}>
                                ...
                                <View style={[globle_style.form_info, { paddingHorizontal: 10 }]}>
                                    {/* radio btn : */}
                                    <View style={[globle_style.radio_con, globle_style.serv_radio]}>
                                        <View style={[globle_style.serv_rad_wrapp, {}]}>
                                            <TouchableWithoutFeedback onPress={() => handleRsnRadio(1, 'Rainy')}>
                                                <View style={[globle_style.radioWapper, selectRadio === 1 ? globle_style.active_wrapper : globle_style.radioWapper, { marginRight: 0 }]}>
                                        ...
                                        <View style={globle_style.serv_rad_wrapp}>
                                            <TouchableWithoutFeedback onPress={() => handleRsnRadio(2, 'Road Repair')}>
                                                {/* <TouchableWithoutFeedback> */}
                                                <View style={[globle_style.radioWapper, selectRadio === 2 ? globle_style.active_wrapper : globle_style.radioWapper, { marginRight: 0 }]}>
                                                    <View style={globle_style.serv_chos_con}>
                                                        <Repair style={{ marginRight: 11.5 }} />
                                                        <Text style={globle_style.rdo_txt}>Road Repair</Text>
                                                    </View>
                                        ...
                                        <View style={globle_style.serv_rad_wrapp}>
                                            <TouchableWithoutFeedback onPress={() => handleRsnRadio(3, 'Client Called Back')}>
                                                {/* <TouchableWithoutFeedback> */}
                                                <View style={[globle_style.radioWapper, selectRadio === 3 ? globle_style.active_wrapper : globle_style.radioWapper, { marginRight: 0 }]}>
                                                    <View style={globle_style.serv_chos_con}>
                                                        <ClientCall style={{ marginRight: 11.5 }} />
                                                        <Text style={globle_style.rdo_txt}>Client Called Back</Text>
                                                    </View>
                                        ...
                                        <View style={globle_style.serv_rad_wrapp}>
                                            <TouchableWithoutFeedback onPress={() => handleRsnRadio(4, 'Dog Sick')}>
                                                {/* <TouchableWithoutFeedback> */}
                                                <View style={[globle_style.radioWapper, selectRadio === 4 ? globle_style.active_wrapper : globle_style.radioWapper, { marginRight: 0 }]}>
                                                    <View style={globle_style.serv_chos_con}>
                                        ...
                                        <View style={globle_style.serv_rad_wrapp}>
                                            <TouchableWithoutFeedback onPress={() => handleRsnRadio(5, 'Other')}>
                                                {/* <TouchableWithoutFeedback> */}
                                                <View style={[globle_style.radioWapper, selectRadio === 5 ? globle_style.active_wrapper : globle_style.radioWapper, { marginRight: 0 }]}>
                                                    <View style={globle_style.serv_chos_con}>
                                                        <Other style={{ marginRight: 11.5 }} />
                                                        <Text style={globle_style.rdo_txt}>Other</Text>
                                                    </View>

                                                    <View style={[globle_style.static_radio_circle, globle_style.radio_static, selectRadio === 5 ? globle_style.static_radio_circle : globle_style.radio_static]}>
                                                        {
                                                            selectRadio === 5 ? <View style={globle_style.radio_bg}></View> : null
                                                        }
                                                    </View>
                                                </View>
                                            </TouchableWithoutFeedback>
                                        </View>
                                        <TouchableWithoutFeedback onPress={() => handleEndWalkAPi(data.service_time, data.booking_id, data.pet_id)}>
