...
// error message status 
      ...
      const [boardHomeError , setboardHomeError] = useState(false);
      const [foodProvideError , setfoodProvideError] = useState(false);
    ...
    // }
     const handleValidation = async () => {
        ...
        if (boardHome == null || boardHome == '') {
            setboardHomeError('Please select any Option');
        } else {
            setboardHomeError(''); // Clear error if name is valid
        }

       if (foodProvide == null || foodProvide == '') {
            setfoodProvideError('Please select any Option');
        } else {
            setfoodProvideError(''); // Clear error if name is valid
        }
        ...
        if (boardingCertified == null || boardingCertified == '') {
            setcertificateError('Please select any Option');
        } else {
            setcertificateError(''); // Clear error if dob is valid
        }


        if (boarding == null || boarding == '') {
            setboardingError('Please select any Option');
        } else {
            setboardingError(''); // Clear error if serviceAddress is valid
        }

       if (!emergencyName) {
              setemergencyNameError('Please enter your name');
          } else if (!/^[a-zA-Z\s]+$/.test(emergencyName)) {  // Checks if name contains only letters and spaces
              setemergencyNameError('Name can only contain letters and spaces');
          } else if (emergencyName.length <3) {  
              setemergencyNameError('Must be atlest 3 characters ');
          } else {
        ...
        if (!emergencyNumber ) {
                setemergencyNumberError('Please enter your phone number');
            } else if (!/^\d{10}$/.test(emergencyNumber)) {  // Checks if number is exactly 10 digits
        ...
        if (radius && aggresive && dob && certificate && accidental&&emergencyName&&emergencyNumber) {
            updateServiceData();
        }
    ...
    return (
        ...
        <ScrollView style={globle_Style.container}>
            <View style={globle_Style.per_dtl_sec}>
                ...
                <View style={[globle_Style.form_info ,{marginBottom:boardingError?2:10}]}>
                    {/* radio btn : */}
                    ...
                    {
                        boardingError ? <Text style={[globle_Style.errorText,{paddingHorizontal:5,}]}>{boardingError}</Text> : null
                    }
                </View>

                <View style={[globle_Style.form_info ,{marginBottom:boardHomeError?2:10}]}>
                    {/* radio btn : */}
                    ...
                    {
                        boardHomeError ? <Text style={[globle_Style.errorText,{paddingHorizontal:5,}]}>{boardHomeError}</Text> : null
                    }
                </View>

                <View style={[globle_Style.form_info ,{marginBottom:foodProvideError?2:10}]}>
                    {/* radio btn : */}
                    ...
                    {
                        foodProvideError ? <Text style={[globle_Style.errorText,{paddingHorizontal:5,}]}>{foodProvideError}</Text> : null
                    }
                </View>
