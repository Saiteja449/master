...
const verifyBankOtp = ({ route, navigation }) => {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      if (isConnected) {
        ...
        const url = `${API_BASE_URL}provider/verifyWithdrawal`;
        ...
        if (result.status == true) {
          Alert.alert("Withdrawal requested successfully!!", "It will be initiated within 24-48hrs to your bank account.", [
  ...
  return (
    <View style={globle_Style.login_sec}>
      <View style={globle_Style.login_con}>
        ...
        <Text style={[globle_Style.login_para, {marginTop:15}]}>
          Please enter the 4 - digit verification code sent to you at {'\n'}+91{' '}
          {number}
        </Text>
      </View>
      ...
      <TouchableOpacity onPress={() => handleVerifyOtp()}>
        <View style={globle_Style.globle_btn}>
          <LinearGradient
            ...
            <Text style={globle_Style.gbl_btn}>Verify</Text>
          </LinearGradient>
        </View>
      </TouchableOpacity>
