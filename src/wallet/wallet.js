...
import { ScrollView, Text, TextInput, TouchableWithoutFeedback, View, FlatList, Alert, Modal, TouchableOpacity } from 'react-native';
...
const WalletTransactionItem = ({ title, time, amount, item }) => {
    return (
        <View style={globle_Style.noti_lst}>
            ...
            <View style={[globle_Style.noti_lst_lft, { backgroundColor: '#F5D3C8', marginLeft: 0 }]}>
                {/* {icon} */}
                <DogWalk />
            </View>
            <View style={globle_Style.noti_lst_rgt}>
                <View style={globle_Style.noti_lst_info}>
                    <Text style={[globle_Style.noti_lst_txt, { fontSize: 12 }]}>{title}</Text>
                    <Text style={globle_Style.noti_time_txt}>{time}</Text>
                </View>
                <View style={globle_Style.noti_lst_amont}>
                    <Text style={[globle_Style.noti_amont_txt, { color: item.transaction_type === 'debit' ? '#EC131F' : '#00BC71' }, { fontSize: 13 }]}>{item.transaction_type === 'debit' ? '-' : '+'} {amount}</Text>
                </View>
            </View>
        </View>
    );
...
const WalletScreen = () => {
    ...
    const withdrawAmount = async () => {
        ...
        if (!bank) {
            Alert.alert("Add bank account to withdraw!!", "Please add bank details to withdraw.", [
        ...
        if (bankId === '') {
            Alert.alert("Select bank account!!", "Please select bank account to withdraw.", [
                { text: "OK", }
            ]);
            return
        }
        ...
        if (paymntStatus === 'in_progress') {
            Alert.alert("Withdrawal is in progress!!", "It will be initiated within 24-48hrs to your bank account.", [
                { text: "OK", }
            ]);
            return
        }
        ...
        try {
            ...
            if (result.status == true) {
                navigation.navigate('verifyBankOtp', { amount: withdrawalAmount, number: userData.phone })
        ...
        return (
            ...
            <View style={globle_Style.serv_rad_wrapp}>
                <TouchableWithoutFeedback onPress={() => handleSelectBank(item.id)}>
                    <View style={[globle_Style.radioWapper, { height: 'auto', marginLeft: 16 }, selectRadio === item.id ? globle_Style.active_wrapper : globle_Style.radioWapper, { height: 'auto', marginLeft: 16 }]}>
                        <View style={globle_Style.bank_info}>
                            <View style={globle_Style.bank_infoname}>
                                ...
                                <View style={globle_Style.bank_accontname}>
                                    <Text style={globle_Style.bank_acconttxt}>{formatNumber(item.account_number)}</Text>
                                    <Text style={[globle_Style.bank_acconttxt, { fontSize: 11 }]}>{item.bank_name}</Text>
                                </View>
                            </View>
                            <View style={globle_Style.bank_code}>
                                ...
                                <View style={globle_Style.bank_codergt}>
                                    <Text style={[globle_Style.bank_acconttxt, { fontSize: 11 }]}>: {item.ifsc_code}</Text>
                                    <Text style={[globle_Style.bank_acconttxt, { fontSize: 11 }]}>: {item.account_holder_name}</Text>
                                </View>
                            </View>
                        </View>

                        <View style={[globle_Style.static_radio_circle, globle_Style.radio_static, selectRadio === item.id ? globle_Style.static_radio_circle : globle_Style.radio_static]}>
                            {
                                selectRadio === item.id ? <View style={globle_Style.radio_bg}></View> : null
                            }
                        </View>
                    </View>
    ...
    return (
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} style={globle_Style.container}>
            <View style={globle_Style.wallet_sec}>
                <View style={globle_Style.wallet_con}>
                    <LinearGradient colors={['#2455D8', '#8244E2', '#E732EC']} start={{ x: 1, y: 0 }} style={globle_Style.wallet_grad}>
                        <View style={globle_Style.wallet_lin_con}>
                            <View style={globle_Style.wallet_lin_lft}>
                                ...
                                <Text style={globle_Style.wallet_amont_txt}>₹ {(parseFloat(withdrawalAmount) + parseFloat(inServiceAmount.service_amount)).toFixed(2)}</Text>

                                <Text style={globle_Style.wallet_balnc_txt}>In Service Balance</Text>
                                <Text style={globle_Style.wallet_amont_txt}>₹ {parseFloat(inServiceAmount.service_amount).toFixed(2)}</Text>

                                <Text style={globle_Style.wallet_balnc_txt}>Wallet Balance</Text>
                                <Text style={globle_Style.wallet_amont_txt}>₹ {parseFloat(withdrawalAmount).toFixed(2)}</Text>
                                ...
                                <TouchableWithoutFeedback onPress={() => {
                                    ...
                                    <View style={[globle_Style.walleet_withdral_btn, { opacity: paymntStatus === 'in_progress' ? 0.7 : 1 }]}>
                            ...
                            <View style={globle_Style.wallet_lin_rgt}>
                                <Wallet />
                            </View>
                        </View>
                    </LinearGradient>
                    <View style={globle_Style.wallet_trans}>
                        <View style={globle_Style.wallet_filter}>
                            <View style={globle_Style.wallet_filtr_lft}>
                                <Text style={globle_Style.wallet_filte_txt}>All Transaction</Text>
                            </View>
                            ...
                            <View style={[globle_Style.quot_dtls_con, { borderWidth: 0 }]}>
                                <View style={globle_Style.accrd_con}>
                                    ...
                                    <View style={globle_Style.bank_data}>
                                        ...
                                        <FlatList
                                            data={bankDetails}
                                            renderItem={({ item, index }) => <BankView item={item} index={index} />}
                                            keyExtractor={(item, index) => index.toString()}
                                        ...
                                        <TouchableOpacity onPress={() => {
                                            ...
                                            <View style={globle_Style.add_bank}>
                                                ...
                                                <View style={{ marginBottom: 36 }}>
                                                    <Text style={[globle_Style.add_bank_txt, { marginBottom: 0 }]}>Add New Bank Account</Text>
                                                    <View style={{ height: 1, backgroundColor: '#1D1D1D' }} />
                                                </View>
                                    ...
                                    <TouchableWithoutFeedback onPress={() => { withdrawAmount() }}>
